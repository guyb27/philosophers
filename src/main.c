...
void	ft_create_thread(t_philo_heart **philo_heart, char *str, t_philo_location locate, int x, int y)
{
	pthread_t		thread;
	...
	while (1)
	{
		...
		{
			philo->locate = ft_get_philo_locate(((t_wand*)(*philo_heart)->prev->data)->locate->number, x, y);
...
int main (void)
{
	...
	ft_get_locate(wand_locate, x, y);
	count = -1;
	philo_heart = NULL;
	while (++count < NB_PHILO)
		ft_create_wand(&philo_heart, wand_locate[count]);
	while (--count >= 0)
		ft_create_thread(&philo_heart, philo_name[count], philo_locate[count], x, y);
